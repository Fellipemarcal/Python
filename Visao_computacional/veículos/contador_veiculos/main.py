...
largura_min = 80  # Largura mínima do retângulo
altura_min = 80
offset = 6  # Tolerância
pos_linha = 550
delay = 60
...
# ✅ Verifica se a video.mp4 foi carregada corretamente
...
while True:
    ...
    for (x, y) in detec:
        if pos_linha - offset < y < pos_linha + offset:
            ...
            detec.remove((x, y))
            print("🚗 Veículo detectado:", carros)

    cv2.putText(frame1, "CONTADOR: " + str(carros), (450, 70), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 5)
    cv2.imshow("Vídeo Original", frame1)
