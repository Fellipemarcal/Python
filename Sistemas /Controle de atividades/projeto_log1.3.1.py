...
def dados_agrupados_por_funcionario():
    ...
    for d in dados:
        ...
        if nome not in agrupado:
            agrupado[nome] = {"sep": 0, "conf": 0, "erros": 0}

        agrupado[nome]["sep"] += sep
        agrupado[nome]["conf"] += conf
        agrupado[nome]["erros"] += erros

    resultado = []
    for nome, v in agrupado.items():
        indice = calcular_indice(v["sep"], v["conf"], v["erros"])
        resultado.append((nome, indice))
...
def abrir_filtros():
    ...
    tk.Label(topo, text="Filtrar por Nome ou ID:").grid(row=0, column=0)
    entry_filtro = tk.Entry(topo, width=30)
    ...
    tk.Button(topo, text="Aplicar Filtro", command=aplicar).grid(row=0, column=2, padx=5)
    tk.Button(topo, text="Exportar Excel", command=exportar).grid(row=0, column=3)
...
def atualizar_grafico():
    agrupado = dados_agrupados_por_funcionario()
    nomes = [n for n, _ in agrupado]
    indices = [i for _, i in agrupado]

    ax.clear()
    ax.bar(nomes, indices)
    ax.axhline(META, linestyle="--")
    ax.set_title("Desempenho Geral por Funcionário")
    canvas.draw()
...
def grafico_individual():
    ...
    plt.figure()
    plt.bar(["Separação", "Conferência", "Erros"], [sep, conf, erros])
    plt.title(f"Desempenho Total - {nome}")
    plt.show()
